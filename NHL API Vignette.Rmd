...
TeamStats <- function(name = NULL){
  ...
  return(list(StatsNumers = StatsNums, StatsRank = StatsRank))
...
allcomb <- left_join(alltots, allstat$StatsNumers)
...
f <- ggplot(goal, aes(as.factor(gpbin), mostWinsOneSeason))

f + geom_violin(aes(color = gpbin)) +
